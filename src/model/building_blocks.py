...
'''
---------------------------------------
multimodal bilinear pooling fusion block
---------------------------------------
'''
class MCB(nn.Module):
    def __init__(self, config, prefix=""):
        super(MCB, self).__init__() # Must call super __init__()
        name = prefix if prefix is "" else prefix+"_"
        print("Multimodal Compact Bilinear - ", name)

        idim_1 = config.get(name+"hp_idim_1", -1)
        idim_2 = config.get(name+"hp_idim_2", -1)
        hdim = config.get(name+"hp_hdim", -1) # 512

        self.output_dim = hdim

        # self.fc_1 = nn.Linear(idim_1, hdim)
        # self.fc_2 = nn.Linear(idim_2, hdim)
        # self.fc_3 = nn.Linear(hdim, hdim)

        self.weight1 = nn.Parameter(torch.rand(idim_1, hdim), requires_grad=False)
        self.weight2 = nn.Parameter(torch.rand(idim_2, hdim), requires_grad=False)

        # Initialize weights such that they are positive and sum to 1
        self.weight1.data = (2 * self.weight1.data - 1) / hdim
        self.weight2.data = (2 * self.weight2.data - 1) / hdim
    ...
    def forward(self, inp):
        ...
        """
        x1, x2 = inp[0], inp[1]
        # Project input1 and input2 to higher dimensional space
        x1_projected = F.linear(x1, self.weight1)
        x2_projected = F.linear(x2, self.weight2)

        # Compute the FFT on both
        x1_fft = torch.fft.rfft(x1_projected)
        x2_fft = torch.fft.rfft(x2_projected)

        # Element-wise product in the Fourier domain (equivalent to outer product in time domain)
        x_product_fft = x1_fft * x2_fft

        # Inverse FFT to get back to time domain
        x_product = torch.fft.irfft(x_product_fft, n=self.output_dim)

        # Sum pool the result if required
        # if self.sum_pool:
        #     x_product = x_product.sum(dim=1)
...
class LocalGlobalVideoTextInteractions(nn.Module):
    def __init__(self, config, prefix=""):
        ...
        if self.mm_fusion_method == "mul":
            '''
            Multi-modal Compact Bilinear Fusion
            '''
            # self.fusion_fn = self._make_modulelist(
            #         MCB(config, "lgi"), self.nse)

            '''
            action is important: action augmentation fusion block
            '''
            fusion_list = nn.ModuleList()
            fusion_list.append(HadamardProduct(config,"lgi"))
            fusion_list.append(MCB(config,"lgi"))
            fusion_list.append(HadamardProduct(config,"lgi"))
            ...
            self.fusion_fn = fusion_list

            # # action augmentation block
            # fuse_1st = self._make_modulelist(HadamardProduct(config, 'lgi'), 1)
            # fuse_2nd = fuse_1st.append(MCB(config, 'lgi'))
            # fuse_3rd = fuse_2nd.append(HadamardProduct(config, 'lgi'))
            # self.fusion_fn = fuse_3rd
        ...
        self.scl = SoftContrastiveCriterion(config)
    ...
    def _segment_level_modality_fusion(self, s_feats, se_feats):
        ...
        # fuse segment-level feature with individual semantic entitiey features(entity, action and event)
        m_feats = []
        for n in range(self.nse):
            ...
            if self.mm_fusion_method == "concat":
                ...
                scl_loss = self.scl([s_feats,q4s_feat])
            else:
...
# class SoftContrastiveCriterion(nn.Module):
#     def __init__(self, cfg, prefix=""):
#         super(SoftContrastiveCriterion, self).__init__()
...
soft contrastive learning for representation alignment

L_{ij} = -\log \left( \frac{\exp(-\alpha \cdot d_{ij})}{\sum_{k=1}^{N} \mathbb{1}_{[y_k=y_i]} \cdot \exp(-\alpha \cdot d_{ik})} \right)
...
class SoftContrastiveCriterion(nn.Module):
    ...
    def forward(self, inp):
        ...
        # Reshape x1 and x2 if necessary
...
class TGRegressionCriterion(nn.Module):
    ...
    def forward(self, net_outs, gts):
        ...
        alpha = 1e-4
        # print("scl: ", scl_loss)
        total_loss = self.regloss1(loc[:,0], s_gt) + self.regloss2(loc[:,1], e_gt) + alpha*scl_loss
